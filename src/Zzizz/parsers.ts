...
export class ZizParser {
  ...
  async parseMangaDetails(
    ...
    const synopsis: string = Application.decodeHTMLEntities(
      $(SELECTORS.MANGA.SYNOPSIS).next(".prose").text().trim() || "No description available",
    );

    const shareUrl: string = `${source.domain}/manga/${mangaId}/`;

    // Cover image is always in the left column; build full URL if needed
    const image: string = $(SELECTORS.MANGA.COVER).first().attr("src") || "";
    ...
    const tagGroups: TagSection[] = [{ id: "genres", title: "Genres", tags: genres }];
  ...
  parseChapterList(
    ...
    $(SELECTORS.CHAPTER.ITEMS).each((_, obj) => {
      ...
      const chapterTitle = $(obj).find(SELECTORS.CHAPTER.TITLE).text().trim();
      const { number: chapNum, name: chapName } = ZizHelpers.extractChapterNumber(
        chapterTitle,
        chapterId,
      );
      ...
      const dateMatch = dateText.match(/(\d+\s+(?:day|hour|minute)s?\s+ago)/i);
   ...
   */
  async parseChapterDetails($: CheerioAPI, chapter: Chapter): Promise<ChapterDetails> {
    const pages: string[] = [];
    ...
    if (canvases.length > 0) {
      ...
      $("script").each((_, script) => {
        ...
        const apiMatch = scriptContent.match(/pageApiUrlBase\s*=\s*["']([^"']+)["']/);
        if (apiMatch) {
  ...
  async parseSearchResults($: CheerioAPI): Promise<SearchResultItem[]> {
    ...
    $(SELECTORS.SEARCH.RESULTS).each((_, obj) => {
      ...
      const image = $(obj).find(SELECTORS.SEARCH.IMAGE).first().attr("src") || "";
      if (!title || !href) return;
      const mangaId = ZizHelpers.cleanMangaId(href);
      const fullImageUrl = ZizHelpers.buildFullUrl(image, "https://zzizz.xyz");
      results.push({
        mangaId: mangaId,
    ...
    switch (section.id) {
      case "latest_updates":
        ...
        $(SELECTORS.DISCOVER.LATEST_UPDATES).each((_, obj) => {
          ...
          const firstChapterLink = $(obj).find("div.mt-3.space-y-2 > a").first();
          let subtitle = undefined;
          if (firstChapterLink.length > 0) {
            const chapterText = firstChapterLink.find("p.text-white").text().trim();
      ...
      default:
        ...
        $(".manga-item, .manga-card").each((_, obj) => {
          const title = $(obj).find("h3, .title, .manga-title").text().trim();
          const image = $(obj).find("img").first().attr("src") || "";
          const href = $(obj).find("a").first().attr("href") || "";
          // Find chapter count in a <p> or <span> containing 'Chapters'
          let subtitle = undefined;
  ...
  public idCleaner(url: string): string {
    ...
    return match ? match[1] : cleanUrl.split("/").filter(Boolean).pop() || "";
  }
}
