...
const CONSTANTS = {
  ...
  SORTING: {
    VALID_SORTS: ["", "latest", "popular", "rating", "name_asc", "name_desc"],
  },
  SELECTORS: {
    SEARCH_MANGA: ".content-grid a[href*='/manga/']",
    PAGINATION_NEXT: ".pagination a[title='Next Page'], .pagination a:contains('Next')",
    PAGINATION_NUMBERS: ".pagination .page-link",
  },
} as const;

// Helper function to handle Cloudflare and redirect protection detection
async function safeRequest(requestOptions: Request): Promise<[Response, ArrayBuffer]> {
  const [response, buffer] = await Application.scheduleRequest(requestOptions);
  if (response.status === 403 || response.status === 307) {
    throw new CloudflareError(requestOptions, "Cloudflare protection detected (403 Forbidden)");
  }
    ...
    SettingsFormProviding
{
  ...
  async getSearchFilters(): Promise<SearchFilter[]> {
    ...
    $("a[href*='genre'], .genre-filter a, .filter-genre a, a:contains('Genres')").each((_, el) => {
      ...
      if (href.includes("genre=")) {
        ...
        id = match ? decodeURIComponent(match[1]) : label.toLowerCase().replace(/\s+/g, "-");
      } else {
   ...
   */
  private buildSearchFilters(genres: { id: string; value: string }[]): SearchFilter[] {
    return [
      {
        type: "multiselect",
        id: "genre",
  ...
  private extractSearchParams(
    ...
    if (sortingMethod?.id) {
      ...
      orderValue = (queryObj.sort as string) || (queryObj.order as string) || "";
    }
  ...
  async getSearchResults(
    ...
    if (paginationNav.length > 0) {
      ...
      const nextButton = paginationNav.find("a[title='Next Page'], a:contains('Next')");
  ...
  public async getSortingOptions(): Promise<{ id: string; label: string }[]> {
    ...
    try {
      ...
      $("a[href*='sort'], .sort-filter a, .filter-sort a, a:contains('Sort By')").each((_, el) => {
        const href = $(el).attr("href") || "";
        const label = $(el).text().trim();
        ...
        if (href.includes("sort=")) {
          ...
          id = match ? decodeURIComponent(match[1]) : this.mapSortLabelToId(label);
        } else {
  ...
  private mapSortLabelToId(label: string): string {
    ...
    if (labelLower.includes("most recent") || labelLower.includes("recent")) {
      return "latest";
    } else if (labelLower.includes("most popular") || labelLower.includes("popular")) {
      return "popular";
    } else if (labelLower.includes("best rating") || labelLower.includes("rating")) {
      return "rating";
    } else if (labelLower.includes("name (a-z)") || labelLower.includes("a-z")) {
      return "name_asc";
    } else if (labelLower.includes("name (z-a)") || labelLower.includes("z-a")) {
      return "name_desc";
    } else {
