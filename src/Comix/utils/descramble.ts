...
export function readScrambleHeaders(headers: Record<string, string>): ScrambleParams | null {
  ...
  return { seed, cols: grid.cols, rows: grid.rows, algo: Number.isFinite(algo) ? algo : 2 };
}
...
export async function descrambleImage(
  ...
  if (algo === 3) {
    ...
    lookup = computeDescrambleLookupAlgo3(seed);
  } else {
    lookup = computeDescrambleLookup(seed, cols * rows);
  }
