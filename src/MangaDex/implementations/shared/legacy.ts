...
// Anchored v4 UUID locked to version 4, so a corrupt id cannot
// inject extra path segments into a request URL.
export const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
// Unanchored variant for UUIDs embedded in user typed search text.
export const UUID_SEARCH_RE =
...
async function resolveLegacyId(id: string, type: "manga" | "chapter"): Promise<string> {
  ...
  promise = (async (): Promise<string> => {
    try {
      ...
      // Lowercase to match the form we used for direct UUIDs above.
      const newId = rawNewId.toLowerCase();
...
export async function resolveMangaIds(ids: string[]): Promise<Record<string, string>> {
  ...
  for (const id of ids) {
    ...
    if (Number.isFinite(num)) toFetchSet.add(num);
  }
  ...
  if (toFetch.length > 0) {
    batchResolved = (async () => {
      ...
      const responses = await Promise.all(
        chunk(toFetch, 100).map((batch) =>
          fetchJSON<LegacyMappingResponse>({
            url: buildLegacyMappingUrl(),
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: { type: "manga", ids: batch },
          }),
        ),
      );
      ...
      for (const item of responses.flatMap((r) => r.data ?? [])) {
        ...
        if (legacyId !== undefined && newId) result[legacyId.toString()] = newId.toLowerCase();
      }
      return result;
    })();
