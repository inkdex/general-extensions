...
const SYNTHETIC_RATING_TAGS: Tag[] = [
  { id: "1", group: "content_rating", name: { en: "Safe" } },
  { id: "2", group: "content_rating", name: { en: "Suggestive" } },
  { id: "3", group: "content_rating", name: { en: "Erotica" } },
  { id: "4", group: "content_rating", name: { en: "Pornographic" } },
];
...
export const SYNTHETIC_RATING_ID_TO_NAME: Readonly<Record<string, string>> = Object.fromEntries(
  SYNTHETIC_RATING_TAGS.map((tag) => [tag.id, tag.name.en.toLowerCase()]),
);
...
function startFetch(): Promise<TagCache> {
  ...
  // finally() observes both outcomes so a rejection is not unhandled.
...
export async function getSearchTagSections(enabledRatings?: string[]): Promise<TagSection[]> {
  ...
  for (const tag of cache.tags) {
    ...
    const blocked =
      ...
      !enabledRatings.includes(tag.name.en.toLowerCase());
    section.tags?.push({
      id: tag.id,
