...
export const parseChapters = ($: CheerioAPI, sourceManga: SourceManga): Chapter[] => {
  ...
  for (const chapterObj of arrChapters) {
    const chapterId: string = $(chapterObj).attr("href")?.split("/")[2] ?? "";
...
export const parseRecentSection = async ($: CheerioAPI): Promise<DiscoverSectionItem[]> => {
  ...
  for (const recentObj of $("> div", $(".grid-cols-2").first()).toArray()) {
    const id = $("a.text-secondary", recentObj).attr("href")?.split("/")[2] ?? "";
    ...
    const chapterId = $("a.relative.block").attr("href")?.split("/")[2] ?? "";
...
export const parseTrendingSection = async ($: CheerioAPI): Promise<DiscoverSectionItem[]> => {
  ...
  for (const trendingObj of $("> div", $(".grid-cols-2").last()).toArray()) {
    const id = $("a.relative", trendingObj).attr("href")?.split("/")[2] ?? "";
    const title = $("div a > div", trendingObj).first().text().trim() ?? "";
    const imageUrl =
      $("a img", trendingObj).attr("src") ?? $("a img", trendingObj).attr("data-src") ?? "";
...
export const parseSearch = async ($: CheerioAPI): Promise<SearchResultItem[]> => {
  ...
  for (const item of $(".my-3.grid > div").toArray()) {
    ...
    const image = $("a img", item).attr("src") ?? $("a img", item).attr("data-src") ?? "";
