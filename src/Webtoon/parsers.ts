...
export abstract class WebtoonParser extends WebtoonSettings {
  ...
  parseChaptersList(dto: WebtoonChaptersListDto, sourceManga: SourceManga): Chapter[] {
    return dto.episodeList.map((elem) => this.parseChapter(elem, sourceManga));
  }

  parseChapter(elem: WebtoonChaptersElemDto, sourceManga: SourceManga): Chapter {
    return {
  ...
  parseTodayTitles($: CheerioAPI, allTitles: boolean): PagedResults<SearchResultItem> {
    ...
    for (let i = 0; i <= list.length - 1 && (allTitles || mangas.length < 10); i++) {
      if ($(list[i]).find("strong.title")) mangas.push(this.parseMangaFromElement($(list[i])));
    }
  ...
  parseCanvasFromRecommendedElement(elem: CheerioElement): SearchResultItem {
    const mangaId =
      elem
        ...
        ?.replace(BASE_URL + "/", "") ?? "";
    const subtitle =
      "Canvas" + (this.languages.length > 1 ? " - " + this.languageTitleFromId(mangaId) : "");
    return {
      mangaId: mangaId,
  ...
  parseSearchResults($: CheerioAPI): PagedResults<SearchResultItem> {
    return {
      items: $("#content > div.webtoon_list_wrap ul li a._card_item")
        .toArray()
        .map((elem) => this.parseMangaFromElement($(elem))),
    };
  }
