...
export class ScyllaComicsExtension implements ScyllaComicsImplementation {
  ...
  async getGenreTags(): Promise<TagSection[]> {
    const request: Request = {
      url: new URL(SCYLLA_COMICS_DOMAIN).addPathComponent("manga").toString(),
      method: "GET",
    };
  ...
  async getSearchResults(
    ...
    const url = new URL("https://scyllacomics.xyz")
      .addPathComponent("manga")
      .setQueryItem("title", encodeURIComponent(query.title?.trim() ?? ""))
      .setQueryItem("type", "")
      .setQueryItem("status", "")
  ...
  private applyFiltersToUrl(query: SearchQuery, url: URL): void {
    ...
    const isStrict =
      ...
      (strictVal.toLowerCase() === "on" || strictVal.toLowerCase() === "true");
