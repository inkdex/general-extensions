...
export function parseMangaDetails($: CheerioAPI, mangaId: string, sourceUrl: string): SourceManga {
  // title
  const primaryTitle = Application.decodeHTMLEntities(
    $("h2.text-2xl.font-bold").clone().children("span").remove().end().text().trim() ||
      $("h1.post-title, h1").first().text().trim(),
  );

  // secondary/alt title i dont see where paperback displays this information
  const secondaryTitles: string[] = [];
  const altFromImg = $("img.lazyload").attr("alt")?.trim();
  if (altFromImg) secondaryTitles.push(Application.decodeHTMLEntities(altFromImg));

  const altTitle = $("div.flex.gap-1:has(span:contains('Alternative Titles')) span")
    .last()
    .text()
    .trim();
  if (altTitle) secondaryTitles.push(Application.decodeHTMLEntities(altTitle));

  // thumbnail
  let image =
    $("div.fixed-img img.lazyload").attr("data-src") ?? $("section img").first().attr("src") ?? "";
  if (image.startsWith("/")) image = sourceUrl + image;
  image = encodeURI(image);

  // author
  const author =
    $("p:has(span:contains('Author')) span.capitalize").first().text().trim() ||
  ...
  const rawStatus = $("p:has(span:contains('Status')) span.capitalize").last().text().trim();
  let status = "Ongoing";
...
export function parseChapters($: CheerioAPI, sourceManga: SourceManga): Chapter[] {
  ...
  for (const obj of nodeArray) {
    ...
    if (chapNumRegex) {
      const rawNum = chapNumRegex[1]?.replace(/[-_]/g, ".") ?? chapNumRegex[2] ?? "0";
      chapNum = parseFloat(rawNum) || 0;
    }

    // chapter release date
    const dateText = $("div.flex.justify-between.gap-3 > span.text-gray-500", obj)
      .last()
      .text()
      .trim();
...
export function parseViewMore(
  ...
  for (const a of anchors) {
    ...
    let image = $(a).find("img[data-src]").attr("data-src") ?? $(a).find("img").attr("src") ?? "";
    if (!image) {
      const slide = $(a).closest("swiper-slide");
      if (slide.length) {
        image = slide.find("img[data-src]").attr("data-src") ?? slide.find("img").attr("src") ?? "";
      }
    }
    if (image.startsWith("/")) image = SCYLLA_COMICS_DOMAIN + image;

    let title = $(a).find("img").attr("alt") ?? $(a).find("h2").first().text().trim() ?? "";
    if (!title) {
      const slide = $(a).closest("swiper-slide");
      if (slide.length) {
        title = slide.find("h2").first().text().trim() || slide.find("img").attr("alt") || "";
      }
    }
    if (!title) continue;
    ...
    if (sectionId === "recent_chapters") {
      ...
      const chapterBlock = cardParent.children("div.flex.flex-col").last();
      const chapterText = chapterBlock.find("a b").first().text().trim();
      const timeAgo = chapterBlock.find("span.text-xs").first().text().trim();
  ...
  if (out.length === 0) {
    container.find("swiper-slide").each((_, slide) => {
      ...
      let image =
        $slide.find("img[data-src]").attr("data-src") ?? $slide.find("img").attr("src") ?? "";
      if (image.startsWith("/")) image = SCYLLA_COMICS_DOMAIN + image;

      const title = $slide.find("h2").first().text().trim() || $slide.find("img").attr("alt") || "";
      if (!title) return;
...
export async function getFeaturedSectionItems(): Promise<PagedResults<DiscoverSectionItem>> {
...
export async function getMostPopularSectionItems(
  ...
  const currentPage =
    parseInt($("li.pagination-link.pagination-active span").text().trim()) || page - 1;
...
export async function getRecentlyAddedSectionItems(
  ...
  const currentPage =
    parseInt($("li.pagination-link.pagination-active span").text().trim()) || page;
  const nextPageExists =
    $("li.pagination-link").filter((_, el) => {
      const txt = $(el).text().trim();
      return /^\d+$/.test(txt) && parseInt(txt) === currentPage + 1;
...
export async function getRecentChaptersSectionItems(
  ...
  const page = metadata?.page ?? 1;
  const request: Request = {
    url: page > 1 ? `${SCYLLA_COMICS_DOMAIN}/?page=${page}` : SCYLLA_COMICS_DOMAIN,
    method: "GET",
  };
  const $ = await fetchCheerio(request);
...
export function parseGenreTags($: CheerioAPI): TagSection[] {
  ...
  $("div.flex.items-center.gap-2").each((_, el) => {
    const title = Application.decodeHTMLEntities($("label", el).text().trim());
    const rawId = $("input", el).attr("id") ?? title;

    if (!title || !rawId) return;

    const id = Application.decodeHTMLEntities(rawId).replace(/[^a-zA-Z0-9_-]/g, "_"); // sanitize for Paperback
...
export function parseSearch($: CheerioAPI, baseUrl: string): SearchResultItem[] {
  ...
  $("div#card-real").each((_, obj) => {
    ...
    const title: string =
      $("img.lazyload", obj).attr("alt") ?? $("h2.text-sm.font-semibold", obj).text().trim() ?? "";
